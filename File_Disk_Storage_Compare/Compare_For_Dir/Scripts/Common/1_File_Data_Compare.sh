...
if [ $? -eq 0 ];then
  if [ "$src_dir" = "$dest_dir" ];then
    ...
    src_dir1=`echo ${src_dir} | awk 'BEGIN{FS="/"}''{print $NF}'`
    dest_dir1=`echo ${dest_dir} | awk 'BEGIN{FS="/"}''{print $NF}'`
    if [ "$src_dir1" = "$dest_dir1" ];then
      dir_name=$src_dir1
      myLog 2 "compare_dir_name:$dir_name" |tee -a $(dirname $(dirname $CURRENT_DIR))/Logs/info.log
      sshpass -p${dest_password} scp "${dest_username}"@"${dest_ip}":/tmp/${dest_ip}dest_file_md5.list $file_out_dir/  & PIDIOS=$!
      sshpass -p${src_password} scp "${src_username}"@"${src_ip}":/tmp/${src_ip}src_file_md5.list $file_out_dir/  & PIDMIX=$!
      wait $PIDIOS
      wait $PIDMIX
      if [ $? -eq 0 ];then
        cat $file_out_dir/${src_ip}src_file_md5.list |awk -F' ' '{print $1}' > $file_out_dir/${src_ip}src_file_md5_1.list && cat $file_out_dir/${src_ip}src_file_md5.list | awk -F "${dir_name}" '{print $2}' > $file_out_dir/${src_ip}src_file_md5_2.list && awk 'NR==FNR{a[i]=$0;i++}NR>FNR{print a[j]" "$0;j++}' $file_out_dir/${src_ip}src_file_md5_1.list $file_out_dir/${src_ip}src_file_md5_2.list > $file_out_dir/${src_ip}src_file_md5_3.list
        cat $file_out_dir/${dest_ip}dest_file_md5.list |awk -F' ' '{print $1}' > $file_out_dir/${dest_ip}dest_file_md5_1.list && cat $file_out_dir/${dest_ip}dest_file_md5.list | awk -F "${dir_name}" '{print $2}' > $file_out_dir/${dest_ip}dest_file_md5_2.list && awk 'NR==FNR{a[i]=$0;i++}NR>FNR{print a[j]" "$0;j++}' $file_out_dir/${dest_ip}dest_file_md5_1.list $file_out_dir/${dest_ip}dest_file_md5_2.list > $file_out_dir/${dest_ip}dest_file_md5_3.list
      ...
      myLog 4 "please make sure compare top dirs are same" |tee -a $(dirname $(dirname $CURRENT_DIR))/Logs/error.log
    fi
  fi
else
