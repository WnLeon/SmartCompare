...
# Function: Compare Parameters for mysql
# Version: V1.1
# Update: 2023-2-23
...
#校验文件存储位置
file_out_dir=$task_out_dir/"1_Parameters_Compare"
...
mysql -u$src_username -p$src_password -P$src_port -h$src_ip  -N -e "show variables;" > $file_out_dir/src_variables
sed -e '/pseudo_thread_id/d' -e '/timestamp/d' $file_out_dir/src_variables > $file_out_dir/src_variables_cut

mysql -u$dest_username -p$dest_password -P$dest_port -h$dest_ip  -N -e "show variables;" > $file_out_dir/dest_variables
sed -e '/pseudo_thread_id/d' -e '/timestamp/d' $file_out_dir/dest_variables > $file_out_dir/dest_variables_cut

diff $file_out_dir/src_variables_cut $file_out_dir/dest_variables_cut > $file_out_dir/diff_out

if [ $? -eq 0 ];then
  myLog 2 "Parameters check passed" | tee -a $(dirname $(dirname $CURRENT_DIR))/Logs/info.log
else
  myLog 4 "Parameters check failed, please review $file_out_dir/diff_out" |  tee -a $(dirname $(dirname $CURRENT_DIR))/Logs/error.log
fi