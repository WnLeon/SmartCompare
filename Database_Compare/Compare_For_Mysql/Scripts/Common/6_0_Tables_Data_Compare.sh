...
do
{
        ...
        {
        #唯一键所在列名
        unique_column=`mysql -u$src_username -p$src_password -P$src_port -h$src_ip  -N -e "select COLUMN_NAME from information_schema.COLUMNS where TABLE_SCHEMA='$db_database' and TABLE_NAME ='$table_name' and COLUMN_KEY='PRI' limit 1;" |sed  's/,/\`,\`/g' | sed 's/^/\`&/g' | sed 's/$/&\`/g' 2>/dev/null`
        ...
        mysql -u$src_username -p$src_password -P$src_port -h$src_ip -D$db_database  -N -e "select $unique_column,md5(concat_ws(',',$col_list,',')) from $db_database.$table_name order by $unique_column;" >>$file_out_dir/${db_database}_${table_name}/sorc;
        if [ $? -ne 0 ]; then
