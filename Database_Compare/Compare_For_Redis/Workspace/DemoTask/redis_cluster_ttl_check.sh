...
function init_redisttlcompare_pipe_conf()
{
    for i in `seq $slot_num`;
    do
      if [ ! -d s${i} ];then
    ...
    do
        ...
        sed -i "s/src_redis_ip=1.1.1.1/src_redis_ip="${src_redis_ip}"/g" $workdir/s${i}/redis_ttl_check_pipe.sh
        sed -i "s/src_redis_port=6666/src_redis_port="${src_redis_port}"/g" $workdir/s${i}/redis_ttl_check_pipe.sh
        sed -i "s/src_redis_passwd=/src_redis_passwd="${src_redis_passwd}"/g" $workdir/s${i}/redis_ttl_check_pipe.sh

        sed -i "s/dest_redis_ip=2.2.2.2/dest_redis_ip=${dest_redis_ip}/g" $workdir/s${i}/redis_ttl_check_pipe.sh
        sed -i "s/dest_redis_port=7777/dest_redis_port=${dest_redis_port}/g" $workdir/s${i}/redis_ttl_check_pipe.sh
        sed -i "s/dest_redis_passwd=/dest_redis_passwd=${dest_redis_passwd}/g" $workdir/s${i}/redis_ttl_check_pipe.sh
    done
...
function execttl_pipe()
{
    echo "`date '+%Y-%m-%d %H:%M:%S'` [INFO] s$i keys collect start..." | tee -a $log_file && cd $workdir/s${i} && bash $workdir/s${i}/redis_ttl_check_pipe.sh
}
...
function redis_ttl_compare()
{
    ...
    if [ $? -eq 0 ];then
      touch $resultdir/src_cluster_ttlkey_new.log
      cat $src_cluster_ttlkey > $resultdir/src_cluster_ttlkey.log
      sort -r $resultdir/src_cluster_ttlkey.log -o $resultdir/src_cluster_ttlkey_new.log
      rm -rf $resultdir/src_cluster_ttlkey.log

      touch $resultdir/dest_cluster_ttlkey_new.log
      ...
      touch $resultdir/src_cluster_ttl_pick_new.log
      ...
      touch $resultdir/dest_cluster_ttl_pick_new.log
